给 OpenClaw 加上一层统一的联网路由策略。

这个 workspace 版本不是原样搬运 Claude Code 的 CDP skill，而是把它改造成：

- `web_search` / MCP 搜索的总控策略
- `web_fetch` 的升级路由器
- `browser` 的使用门禁与分流指引
- `markdown-proxy` 的上游总控协作者
...
- `web-access`：广义联网任务总控
- `markdown-proxy`：文章 / 帖子 / 文档正文提取特化 skill

默认分工：

- “搜一下 / 查一下 / 去网站里看看 / 打开网页 / 登录后操作” -> `web-access`
- “把这个链接正文提出来 / 转 Markdown / 归档文章” -> `markdown-proxy`

原仓库里的 CDP Proxy 脚本和说明仍被保留在 `scripts/` 里，作为参考件；但 OpenClaw 集成模式下优先使用原生 `browser`，而不是把 `cdp-proxy.mjs` 当默认入口。
...
原始 skill 的 CDP 模式需要 **Node.js 22+** 和 Chrome 开启远程调试：
...
安装到 `workspace/skills/web-access` 后，主要看 `SKILL.md` 里的 OpenClaw 路由规则，不要机械照搬下面这些 Claude Code 风格命令。

OpenClaw 中的典型请求：

- “帮我查一下 xxx 最新进展”
- “去官网看看这个产品到底支持什么”
- “打开这个站点，帮我找登录后页面里的某个信息”
- “这个页面是动态渲染的，帮我浏览一下”
- “同时调研这 5 个网站，给我摘要”

如果只是文章正文提取，请直接交给 `markdown-proxy`。

...
```yaml
references:
  from_types: [Document, Note, Message]
  to_types: [Document, Note, Project, Task, Person]
  cardinality: many_to_many

mentions:
  from_types: [Message, Note, Document]
...
- The combined skill adds `LearningRecord`, `OperationalError`, and `FeatureRequest` through `scripts/sync_learning_to_ontology.py bootstrap`.
- Validation currently enforces `required`, `forbidden_properties`, enum checks, relation type/cardinality rules, acyclicity, and Event start/end ordering.
- Prefer extending the schema with `schema-append` instead of overwriting the file.
